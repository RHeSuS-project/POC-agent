...
var app = {
    ...
    storeSubscriptionDataForStatistics: function(obj) {
        var sw=true;
        var i = 0;
        while(app.subscriptionDataForStatistics.length>i && sw)
        {
            if(app.subscriptionDataForStatistics[i].datetime<(Math.round(new Date().getTime())-3600000))
            {
                app.subscriptionDataForStatistics.splice(i,1);
            }
            else
                sw=false;
        }
